...
import com.jian.jianpicturebackend.api.aliyunai.AliYunAiApi;
import com.jian.jianpicturebackend.api.aliyunai.model.CreateOutPaintingTaskRequest;
import com.jian.jianpicturebackend.api.aliyunai.model.CreateOutPaintingTaskResponse;
import com.jian.jianpicturebackend.constant.SpaceConstant;
...
import com.jian.jianpicturebackend.manager.FileManager;
import com.jian.jianpicturebackend.manager.auth.SpaceUserAuthManager;
import com.jian.jianpicturebackend.manager.upload.FilePictureUpload;
import com.jian.jianpicturebackend.manager.upload.PictureUploadTemplate;
import com.jian.jianpicturebackend.manager.upload.UrlPictureUpload;
...
import com.jian.jianpicturebackend.model.enums.PictureReviewStatusEnum;
import com.jian.jianpicturebackend.model.vo.PictureVO;
import com.jian.jianpicturebackend.model.vo.UserVO;
import com.jian.jianpicturebackend.service.PictureService;
import com.jian.jianpicturebackend.mapper.PictureMapper;
import com.jian.jianpicturebackend.service.SpaceService;
import com.jian.jianpicturebackend.service.UserService;
import com.jian.jianpicturebackend.utils.ColorSimilarUtils;
...
//                throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "无权限修改他人的图片");
//            }
            // 校验空间是否一致
        ...
        // 根据inputSource的类型，判断是上传文件还是上传url
        ...
        // 设置空间 id（公共图库统一为 0，保证 ShardingSphere 分片键不为 null）
        picture.setSpaceId(spaceId == null ? SpaceConstant.PUBLIC_SPACE_ID : spaceId);
        // 补充审核参数
        ...
        if (pictureId != null) {
            ...
            // ShardingSphere 不允许 UPDATE 的 SET 中包含分片键 spaceId，置空让 MyBatis-Plus 跳过该字段
        ...
        transactionTemplate.execute(status -> {
            ...
            if (finalSpaceId != null && finalSpaceId != SpaceConstant.PUBLIC_SPACE_ID) {
                // 更新空间的使用额度
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        queryWrapper.eq(nullSpaceId, "spaceId", SpaceConstant.PUBLIC_SPACE_ID);
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        if (ObjUtil.isEmpty(div)) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "获元素失败");
        }
        ...
        if (imgElementList != null) {
            for (Element imgElement : imgElementList) {
                ...
                // 处理图片的地址，防止转移或和对象存储冲突的问题
                // 地址只保留？之前的部分：https://cn.bing.com/th?id=OIP.C94040494926211111_0.jpg&pid=ImgGather
                // https://cn.bing.com/th
...
//        checkPictureAuth(loginUser, oldPicture);
        ...
        transactionTemplate.execute(status -> {
            ...
            // 更新空间的使用额度，释放额度（公共图库图片 spaceId 为 0，无需更新）
...
//        checkPictureAuth(loginUser, picture);
        ...
        // 阿里云扩图要求输入图每条边在 512~4096 之间，超出范围的图片用 COS 实时处理调整后再传给阿里云
        String imageUrl = picture.getUrl();
        if (picture.getPicWidth() != null && picture.getPicHeight() != null) {
            int minSide = Math.min(picture.getPicWidth(), picture.getPicHeight());
            int maxSide = Math.max(picture.getPicWidth(), picture.getPicHeight());
            if (minSide < 512) {
                // 太小：放大到短边 512（注意 imageMogr2 默认只缩小不放大，必须用 !WxHr 强制放大短边）
                imageUrl = imageUrl + "?imageMogr2/thumbnail/!512x512r";
            } else if (maxSide > 4096) {
                // 太大：缩小到长边不超过 4096
                imageUrl = imageUrl + "?imageMogr2/thumbnail/4096x4096";
            }
        }
